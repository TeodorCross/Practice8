...
import re
EMAIL = re.compile(r'([a-z0-9_\.]+)@([f\a-z0-9]+\.[a-z]+)')
...
email_parse('ted200798@mail.ru')
email_parse('ted200798@mailru')
...
def val_checker(decorator_check_finc):
    def _val_checker(func_calc_cube):
        @wraps(func_calc_cube)
        def wrapped(x):
            if decorator_check_finc(x):
